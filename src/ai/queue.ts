...
function sqlFormat(sql: string): string {
  if (USE_SQLITE) {
...
export async function addToQueue(examQuestionId: number, studentId: number): Promise<void> {
...
export async function processQueue(limit: number = 10): Promise<number> {
  ...
  for (const job of result.rows as QueueJob[]) {
    try {
