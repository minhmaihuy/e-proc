...
import { tenantDomainForSlug } from '../tenantContext.js';
import { serializeAllowedRecordModes } from '../services/recordingPolicy.js';
import { normalizeIdentityMode } from '../services/identityPolicy.js';
import { TenantInput, validateTenantInput } from '../services/tenantConfigurationPolicy.js';
...
function normalizeTenantInput(body: unknown, existing?: Partial<TenantRow>): TenantInput {
  ...
  const identityVerification = normalizeIdentityMode(
    input.identity_verification ?? input.identityVerification ?? existing?.identity_verification ?? 'off',
  );
  return {
...
router.post('/', requireSuperAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const validationError = validateTenantInput(input, 'create');
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const validationError = validateTenantInput(input, 'update');
    if (validationError) return res.status(400).json({ error: validationError });
...
router.post('/:id/approve', requireSuperAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (!tenant) return res.status(404).json({ error: 'Tenant not found.' });
    const validationError = validateTenantInput(normalizeTenantInput(tenant, tenant), 'approval');
    if (validationError) return res.status(400).json({ error: validationError });
