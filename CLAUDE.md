...
- Live monitoring is optional WebRTC signaling only: matching `tenant_admin` may concurrently view all active regular recorded attempts in a batch through the application's same-origin WebSocket topics, each scoped by trusted tenant and hashed JTI. Short-lived HS256 signaling tokens are sent only in `Sec-WebSocket-Protocol`; validate Origin, topic, rate limit, and heartbeat. Audit metadata belongs only in the assessment plane; never store or log media, SDP, ICE, or signaling payloads. Practice batches, regular `admin`, superadmin, inactive attempts, revoked recording, and disabled configuration fail closed. No hosted broker, STUN, or TURN provider is used.
