...
| Production frontend build failed while loading `@tailwindcss/oxide` | Tailwind v4 uses a platform-native optional package, but deploy installed dev dependencies without overriding an environment/npm configuration that omitted optional dependencies | Keep the Linux x64 GNU binding explicit in client `optionalDependencies`; deploy/setup must use `--include=optional` and run the client native-dependency preflight before Vite | Verify the lockfile contains the Linux binding, both deployment scripts include optional packages, the preflight loads Oxide, and a clean `npm ci` client build passes |
