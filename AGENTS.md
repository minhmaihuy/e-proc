...
- The database is split into three ownership planes. Never configure one plane to fall back to another:
  - **FSA-CLS assessment data-plane:** `src/server/db/postgres.ts`; `DATABASE_URL` in production or `data/eaudit.db` locally. It owns tenant `admin_users` (`tenant_admin`/`admin`), questions, batches, students, submissions, violations, and assessment/AI state. `data_plane_metadata` identifies this database as tenant `fsa-cls`.
  - **Global tenant control-plane:** `src/server/db/controlPlane.ts`; `CONTROL_DATABASE_URL` in production or `data/control-plane.db` locally. It owns the global `superadmin`, tenant settings, provisioning jobs, and tenant audit events. Legacy/staged tenant-account rows may exist only as one-time bootstrap migration input; they are never runtime authentication authority.
...
- Both endpoints use the shared `adminAuthentication.ts` service and return a 24-hour HS256 JWT carrying account id, username, role, and tenant context. Tenant `admin`/`tenant_admin` credentials are read from the current assessment data-plane; global `superadmin` credentials are read from the control-plane. The frontend stores the token, expiry, role, user id, tenant id/slug/name in `localStorage` and sends `Authorization: Bearer <token>`.
- All protected admin routes use `authMiddleware`. It verifies the JWT, reloads tenant accounts from the current assessment database (or superadmin from control), and reloads tenant status/capabilities from control on every request, so deleted accounts, changed roles, or suspended tenants lose effective access immediately.
- Roles are `superadmin`, `tenant_admin`, and `admin`:
  ...
  - `tenant_admin` belongs to exactly one tenant and owns all audit/assessment functions for that tenant, including tenant user management, selecting the actual recording/identity mode for each batch within the superadmin-granted tenant capability, and operational-log lifecycle management. It does not edit the tenant-wide recording allowlist or global tenant/Terraform configuration.
...
- The four current-tenant user CRUD routes at the top of `admin.ts` are protected individually by `requireTenantUserManager`. Immediately after them, `router.use(requireTenantDataAdmin)` globally protects every remaining assessment `/api/admin` route, including AI settings and the read-only recording capability endpoint, so a valid superadmin token cannot fall through into the current server's assessment database. `requireTenantLogManager` separately protects issue lifecycle mutations; `requireSuperAdmin` protects `/api/tenants`. The legacy `requirePlatformAdmin` export aliases `requireTenantDataAdmin` and therefore excludes superadmin.
...
- **Toàn bộ CRUD `/api/admin/users` phải dùng `postgres.ts` của tenant hiện tại.** Mỗi `DATABASE_URL` là tenant boundary; bảng `admin_users` ở đó sở hữu `tenant_admin`/`admin` và cùng plane với các FK `question_bank.uploaded_by`/`batches.created_by`. `controlPlane.ts` chỉ xác thực `superadmin` và cung cấp tenant settings. Startup di chuyển tài khoản tenant cũ từ control về data-plane đúng một lần, giữ nguyên ID; xung đột ID/username phải làm migration dừng thay vì ghi đè.
...
- Recording has two distinct ownership levels: only superadmin grants a tenant-wide allowlist (`tenants.allowed_record_modes`), while only that tenant's `tenant_admin` selects/changes the actual per-batch `batches.record_mode` (`none`, `local`, or `s3`) within the allowlist. Regular `admin` cannot change either level; backend enforcement is authoritative and `record_enabled` remains a compatibility mirror for S3.
...
- **Self-service admin registration has been removed.** The 2026-07 hardening note claimed this, but `GET /is-initialized` and `POST /setup` survived in `admin.ts` above `router.use(authMiddleware)` and stayed reachable **unauthenticated** on production until 2026-08-12 (verified live: `is-initialized` returned `{"initialized":false}` and `setup` with an empty body returned 400, so the handler ran with no auth in front of it). They wrote to the **data-plane** copy of `admin_users`, which no longer backs authentication, so the account could not log in — but it was still an unauthenticated write to the production database, and a full login bypass the moment the mount order in `index.ts` changed. Both routes are deleted and `src/server/routes/adminUserRoutes.test.ts` fails if any route reappears above `authMiddleware`. Instead:
  - The global `superadmin` is seeded automatically in the control-plane by `seedSuperAdmin()` in `src/server/db/controlPlane.ts`, **only when no superadmin exists** — username/password default to `supperadmin` / `superadmin123#2nf`, overridable via the `SUPERADMIN_USERNAME` / `SUPERADMIN_PASSWORD` env vars. Legacy/staged tenant rows do not suppress this seed. **Change this password immediately after first login** (`PUT /api/tenants/change-password`) — the default lives in git history.
  - Other tenant accounts are managed through `GET/POST/PUT/DELETE /api/admin/users` by that tenant's `tenant_admin` only.
  - The first FSA-CLS `tenant_admin` is seeded in the assessment data-plane by `tenantAdminPlaneMigration.ts`, **only when that data-plane has no `tenant_admin` and after any legacy control-plane accounts are copied with their IDs preserved** — defaults `adminfsa` / `adminfsa123#2nf`, overridable via `FSA_TENANT_ADMIN_USERNAME` / `FSA_TENANT_ADMIN_PASSWORD`. **Change this password immediately after first login.**
  - Global tenant control login lives at `/tenant/login` and management lives at `/tenants`; legacy `/admin/tenants` only redirects to management. `/admin/tenant` has been removed and redirects to the tenant dashboard. `/admin/settings` is the separate current-tenant AI provider page and remains available to matching tenant roles; it is not a duplicate tenant-infrastructure workspace. Server-side ownership checks remain authoritative.
...
- **Admin import**: `POST /api/admin/practice/import` (`src/server/routes/admin.ts`) accepts a `.docx` upload (multer memory storage) + optional name, converts with `mammoth.convertToHtml`, stores both HTML and stripped plain text. Managed in `client/src/pages/PracticeManagement.tsx` at `/admin/practice` (list/preview/delete; delete is blocked while any batch references the exam). Batch creation (`BatchManagement.tsx`) has a "Question Bank | Practice" source toggle (`batch/BatchSourceToggle.tsx`); Practice mode swaps the whole blueprint block for `batch/PracticeExamSelect.tsx` and sends `practice_exam_id` instead of `blueprint`, skipping blueprint feasibility checks entirely. **Đã từng thiếu:** hai điểm tích hợp này chưa bao giờ tồn tại cho tới 2026-08-14 dù tài liệu mô tả như đã có, nên Practice hoàn toàn không dùng được — import được đề nhưng không gắn vào đợt thi nào. `scripts/clientBuildDependencies.test.js` đọc source để chốt lại.
- **Create contract**: nút tạo phải áp validation theo nguồn đề. Practice chỉ yêu cầu một `practice_exam_id`, không được bị khóa bởi tổng số câu/module/blueprint bằng 0. `POST /api/admin/batches` dùng `RETURNING id` trong cả SQLite và PostgreSQL và luôn trả `id` số; frontend cần ID này để mở ngay form mời học viên.
...
- **Both student exam pages must statically import the default `CodeEditor`.** `vite-plugin-javascript-obfuscator` runs on each page before Rollup resolves page-local `import()` specifiers; a lazy `import('../components/CodeEditor')` becomes a runtime request to `/components/CodeEditor`, where Nginx returns `index.html` and the browser rejects its `text/html` MIME type. `detectLanguage` already imports the same module, so the lazy form gives no split benefit. `scripts/clientBuildDependencies.test.js` pins the static import in both `StudentExam.tsx` and `StudentPractice.tsx`.
...
The exam can record the candidate's full screen. Since 2026-07-30 the per-batch setting is a **3-value `record_mode`** (`batches.record_mode`, replacing the old boolean `record_enabled` — `tenant_admin`-only, see Admin roles):
...
**Flow of the setting:** `/verify` returns the batch's **effective runtime** `record_mode` (and, for effective `local`, `recording_password`); it flows login → `/confirm` (router state) → `localStorage.recordMode` (+ `localStorage.recordingPassword` for local) → `/exam`. `StudentExam` derives `recordEnabled = recordMode !== 'none'` so all existing recording guards keep working for both `local` and `s3`. Every subsequent server policy check, including verification/start metadata and recording URL/complete/finalize, intersects the stored batch mode with the tenant's current effective allowlist. A revoked or newly-invalid mode is treated as `none` on the next verify/server-gated request without rewriting `batches.record_mode`, preserving batch history. This is not retroactive cancellation: an already-running Local page keeps its verified `localStorage` snapshot until verify/reload, and a presigned S3 PUT URL issued before revocation remains usable until its 15-minute expiry. `record_enabled` remains a backward-compatibility mirror for stored S3 selection, not runtime authority.
...
- **Env required for `s3`:** `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `S3_RECORDINGS_BUCKET`. The supported tenant Terraform module writes the bucket name and manages a private encrypted bucket, origin-scoped PUT CORS, lifecycle, and `GetObject`/`PutObject` IAM on `recordings/*`; manually managed/Vercel environments must provide equivalent controls. `local` mode needs none of these.
...
- Pieces that must stay together:
  ...
  - Both import routes read the Excel `QuestionGroup` column (aliases `Question Group`, `Question Set`, `Bộ đề`) and upsert with `ON CONFLICT (id, question_group)` / `INSERT OR REPLACE` including the group. Essay import keeps the legacy blank-group fallback, but Quiz import must reject a missing header or blank value: allowing a Quiz into `''` makes its set unfilterable and can overwrite a legacy blank-group row. Their "already exists?" probe filters on both columns and uses `?` placeholders — a `$1` there crashes under SQLite.
  ...
  - Deletes take the group: `DELETE /questions/:id?group=…`; bulk-delete accepts `"id|||group"` keys (a plain id still means "all groups", for older clients). `DELETE /questions/question-groups/:group` deletes every question in one named non-empty group. Before an `admin` mutation, check ownership of every matched composite row; a successful response reports non-empty `removedQuestionGroups` whose final question was removed.
...
- `GET /api/admin/questions/question-groups` returns the distinct non-empty groups; `QuestionBank.tsx` uses it for a "Bộ đề" column plus an independent filter that combines with the Module and Type filters. It also lists each non-empty group with its question count, filter action, and a confirmed group-delete action; regular `admin` sees that action enabled only if every group row is theirs, while backend ownership remains authoritative. After deletion it refreshes this derived list (and modules), clears a now-invalid group filter, and uses `id|||group` for row/checkbox identity. Without that column the table is genuinely ambiguous — two rows can show the same ID.
...
- Because the validation-free tenant draft row is live control-plane state, raw draft values are never runtime authority. `resolveTenantEvidencePolicy` derives a fail-closed effective policy on every runtime read: `none` is always safe; granted `local` remains effective without infrastructure or retention validation; granted `s3` is effective only when `recording_retention_days` is an integer from 1 through 365. Identity `off` is always safe; requested `photo` is effective only when both identity and recording retention are integers from 1 through 365 and identity retention is strictly shorter; unsupported `face_match` and invalid values reduce to `off`.
- `tenants.recording_retention_days` is nullable for legacy/never-enabled tenants; superadmin enters 1–365 days at `/tenants`. Approval/provisioning require it for S3. Identity `photo` additionally requires explicit `identity_retention_days` strictly shorter than recording retention. Migration never invents either value.
...
- The **effective**, retention-safe allowlist and identity capability reach data-plane routes through `authMiddleware`, which reloads the tenant row and derives policy per request — `req.adminUser.allowedRecordModes`/identity fields must never expose raw invalid draft capabilities. Do **not** query `tenants` from `admin.ts`; that would cross the control/data-plane boundary.
...
- `GET /api/admin/recording-config` is a read-only effective-capability endpoint. It returns only the current tenant's derived, retention-safe recording/identity capability plus whether the authenticated tenant role may choose a batch mode—not raw draft values. It has no mutation companion and never edits `tenants.allowed_record_modes`. The batch form uses it for UX, while create/update enforcement remains authoritative in the backend.
...
- Delete one grouped question through `DELETE /questions/:id?group=<group>`; bulk-delete uses `id|||group` keys; `DELETE /questions/question-groups/:group` removes all questions in a named non-empty group after one server ownership validation. A plain id remains backward-compatible and means all groups. Groups are derived rather than stored separately, so deleting the final non-empty row removes it from `GET /questions/question-groups` and reports it as `removedQuestionGroups`.
...
- Quiz questions use the separate `POST /api/admin/questions/quiz/import` route, accept `.csv` as well as Excel files, and use the same `(id, question_group)` identity. After a successful Quiz import, the Question Bank resets module/group filters and selects the Quiz category so the imported rows are visible.
- The shared CSV parser removes an optional UTF-8 BOM before SheetJS reads the first header. Without this, a valid BOM-prefixed `ID` is parsed as an empty header and every Quiz row is silently skipped.
- Excel/CSV header column for question group: `QuestionGroup` (aliases also accepted: `Question Group`, `Question Set`, `Bộ đề`). It is mandatory and non-empty for Quiz imports; essay/coding imports retain the legacy empty-string fallback.
...
- Quiz import is separate from essay import: `POST /api/admin/questions/quiz/import`. Its canonical `.csv`/Excel header order is `ID, Type, Level, Topic, QuestionGroup, Question Sample, Option A` through `Option F`, `Correct, Score`; `QuestionGroup` is required for every Quiz row. Supported question types are `SingleChoice` and `MultipleChoice`, with JSON-backed `options`, `correct_answers`, and per-question `score`.
...
- There is no tenant-role infrastructure configuration workspace: legacy `/admin/tenant` redirects to the tenant dashboard. The retained `/admin/settings` page configures tenant-local AI grading and is separate from the superadmin-owned tenant capability/infrastructure form.
...
- `PUT /api/tenants/:id` is deliberately a draft save with no business or infrastructure validation. The frontend update form bypasses native/custom validation, and the backend normalizes and persists even incomplete or legacy-invalid fields so an unrelated change such as enabling Local recording is not blocked by Route53. Superadmin authorization, tenant existence, immutable slug, the Terraform-running lock, pending/not-started reset, and audit remain mandatory. Create and approval validate through `tenantConfigurationPolicy.ts`; approval and provisioning recheck Route53 and the other deployment requirements before AWS work. Validation-free persistence does not imply runtime enablement: authentication, recording configuration, identity, and every student recording path consume only the fail-closed effective evidence policy derived from that live draft row.
...
- `tenants.identity_verification` belongs to the control-plane and safely defaults/backfills to `off`; `batches.identity_verification` belongs to the assessment plane. Only superadmin may request tenant capability and only `tenant_admin` may select it for a batch. Runtime uses the derived effective capability: invalid `photo` retention and unsupported `face_match` are `off`, and the backend rejects a batch mode above that effective tenant mode.
...
- Superadmin configures both `recording_retention_days` and `identity_retention_days` at `/tenants`. New/never-enabled tenants keep both nullable; enabling S3 requires explicit recording retention, while enabling `photo` requires both values and `identity_retention_days < recording_retention_days`. Disabling later preserves selected lifecycles so existing evidence expires instead of triggering destructive bucket removal.
...
| `S3_RECORDINGS_BUCKET` | Rec | — | Private S3 bucket for exam screen recordings; normally written by tenant Terraform from retention configured at `/tenants`. |
...
- **Control/data-plane boundary rule:** global `superadmin` authentication, tenant configuration, Terraform jobs, and tenant audit events use `src/server/db/controlPlane.ts`. Tenant `admin_users` authentication/CRUD plus questions, batches, students, answers, results, violations, and AI grading use `src/server/db/postgres.ts`. Middleware may query each plane separately to combine tenant identity with settings, but SQL must never join across connections.
...
- Live monitoring is optional WebRTC signaling only. A matching `tenant_admin` chooses `batches.live_monitor_mode` per regular recorded batch: `off`, the current self-hosted WebSocket + coturn mode, or the legacy Supabase Realtime + Metered TURN mode. Backend session routes always read that stored value, and reject a change while a candidate is in progress so both peers cannot use different providers. Self-hosted topics are scoped by trusted tenant and hashed JTI and use short-lived HS256 tokens only in `Sec-WebSocket-Protocol`; Supabase private channels use a short-lived ES256 token carrying the same opaque scope. Audit metadata belongs only in the assessment plane; never store or log media, SDP, ICE, or signaling payloads. Practice batches, regular `admin`, superadmin, inactive attempts, revoked recording, and disabled/unconfigured mode fail closed.
