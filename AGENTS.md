...
- `tenants.recording_retention_days` is nullable for legacy/never-enabled tenants; superadmin enters 1–365 days at `/tenants`. Enabling `s3` requires it. Enabling identity `photo` additionally requires explicit `identity_retention_days`, and the backend enforces identity retention is strictly shorter than recording retention. Migration never invents either value.
