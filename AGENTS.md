...
- Live monitoring is optional WebRTC signaling only: matching `tenant_admin` may view one active regular recorded attempt through a private, tenant-and-hashed-JTI scoped Supabase topic. Tokens are short-lived ES256; audit metadata belongs only in the assessment plane; never store or log media, SDP, ICE, payloads, or signaling/private-key secrets. Practice batches, regular `admin`, superadmin, inactive attempts, revoked recording, and missing HTTPS signing config fail closed.
